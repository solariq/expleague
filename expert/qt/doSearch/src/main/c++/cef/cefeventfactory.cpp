...
#if defined(Q_OS_OSX)
    case Qt::Key_Control:
      return (!QCoreApplication::instance()->testAttribute(Qt::AA_MacDontSwapCtrlAndMeta)) ? EVENTFLAG_COMMAND_DOWN
                                                                                           : EVENTFLAG_CONTROL_DOWN;
    case Qt::Key_Meta:
      return (!QCoreApplication::instance()->testAttribute(Qt::AA_MacDontSwapCtrlAndMeta)) ? EVENTFLAG_CONTROL_DOWN
...
CefKeyEvent CefEventFactory::createPressEvent(QKeyEvent* ev) {
  ...
  if (!ev->text().isEmpty()) {
    ...
    event.unmodified_character = ev->text().utf16()[0];
  }
  else {
    event.character = (char16) macKeyCode(event.windows_key_code);
  }
  #endif
