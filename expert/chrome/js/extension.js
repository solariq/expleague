...
DRAGDIS.storage.get("userSettings", function (userSettings) {
    ...
    //Set user settings as as config parameter
    DRAGDIS.config.userSettings = userSettings;

    if (userSettings.showOnboardingNotification) {

        //Flag sidebar to initialize silently (without displaying itself)
        DRAGDIS.config.initHidden = true;
...
DRAGDIS.storage.get("userNotifications", function (userNotifications) {
    ...
    //If user is onboarding, skip all other notifications
    ...
    $.each(userNotifications, function (index, notificaton) {
        
        //Check if notification matches URL pattern
        if (notificaton.Data.includePath) {
            if (notificaton.Data.includePath.indexOf(url) === -1 ) {
                return true;
            }
        }

        if (notificaton.Data.excludePath) {
            if (notificaton.Data.excludePath.indexOf(url) > -1 ) {
                return true;
            }
        }

        //Show notification and return
        if (notificaton.Data.title && notificaton.Data.description) {
            
            //Show first matching notification
            DRAGDIS.config.userSettings.displayNotification = notificaton;

            //Flag sidebar to initialize silently (without displaying itself)
            DRAGDIS.config.initHidden = true;

            //Initialize notification after 3s
            setTimeout(function() {
                DRAGDIS_SIDEBAR.show();
            }, 3000);
        ...
        $('body')
            .on('mouseover', function (event) {
                ...
                if (event.target.childNodes.length) {
                    ...
                    var container = event.target.childNodes[0];
                    ...
                    if (container.classList.contains('uiScaledImageContainer')) {
