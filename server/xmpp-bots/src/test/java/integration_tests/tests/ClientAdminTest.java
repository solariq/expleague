...
public class ClientAdminTest extends BaseSingleBotsTest {
  ...
  public void testClientAdminDialogue() throws JaxmppException {
    ...
    //Assert
    Assert.assertNotNull("room-role-update(none) was not received by admin", initMessages.stream()
        ...
        .findAny()
        .orElse(null));
    Assert.assertNotNull("room-role-update(moderator) was not received by admin", initMessages.stream()
        ...
        .findAny()
        .orElse(null));
    Assert.assertNotNull("room-state-changed(0) was not received by admin", initMessages.stream()
        ...
        .findAny()
        .orElse(null));
    Assert.assertNotNull("offer was not received by admin", initMessages.stream()
        ...
        .findAny()
        .orElse(null));
    Assert.assertNotNull("room-message-received(expert=false) was not received by admin", initMessages.stream()
        ...
        .findAny()
        .orElse(null));
    Assert.assertNotNull("room-message-received(expert=true) was not received by admin", initMessages.stream()
    ...
    Assert.assertNotNull("message from admin was not received by client", messageFromAdmin.getFirstChild("body"));
    Assert.assertEquals("message from admin was not correct", messageFromAdminText, messageFromAdmin.getFirstChild("body").getValue());
  }
}